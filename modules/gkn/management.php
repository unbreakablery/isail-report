<?php 
    ...
    $common->authorizePage("gkn_management");

    $iSAIL_UI->title = $common->getPrefixProjectName() . ' ' . 'Game Key Number Management';

    $data = $_POST;

    $fields = array(
                    array('key' => 'gkn', 'value' => '',    'operator' => '!='),
                    array('key' => 'gkn', 'value' => NULL,  'operator' => 'IS NOT')
                );
                    ...
                    <?php foreach ($users as $user) { ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <button class="btn btn-xs btn-default edit-button" type="button" data-toggle="tooltip" title="" data-original-title="Edit GKN" data-userid="<?php echo $user['userid']; ?>"><i class="fa fa-pencil"></i></button>
                                    <button class="btn btn-xs btn-default remove-button" type="button" data-toggle="tooltip" title="" data-original-title="Remove GKN" data-userid="<?php echo $user['userid']; ?>"><i class="fa fa-trash-o"></i></button>
                                </div>
                            </td>
                        </tr>
                    <?php } ?>
...
<div class="modal fade" id="edit-gkn-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-popout">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-sm btn-default" type="button" id="cancel-edit-gkn">Cancel</button>
                <button class="btn btn-sm btn-primary" type="button" id="save-gkn"><i class="fa fa-check"></i> Update Game Key Number</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="remove-gkn-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-popout">
        <div class="modal-content">
            <div class="block block-themed block-transparent remove-margin-b">
                <div class="block-header bg-primary">
                ...
                <div class="block-content">
                    <div class="text-center text-default">
                        <p><i class="fa fa-question-circle"></i> Would you remove this user GKN really?</p>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    $(document).ready(function() {
        ...
        $('#userTable tbody').on( 'click', 'tr button.edit-button', function () {
                ...
                success:    function( data ) {
                                if (!data.status) {
                                    ...
                                    $.notify({
                                        ...
                                        $("input[name='gkn']").val(data.user.gkn);
                                        ...
                                        $("#edit-gkn-modal").modal({backdrop: 'static'});
                                }
                            }
            });
        ...
        $('#userTable tbody').on( 'click', 'tr button.remove-button', function () {
            targetElement = $(this).closest('tr');
            ...
            $("#remove-gkn-modal").modal({backdrop: 'static'});
        });

        $("#cancel-edit-gkn").click(function() {
            targetElement = null;

            $("#edit-gkn-modal").modal('hide');
        });

        $("#remove-yes-button").click(function() {
            var selectedUser = $(targetElement).find("button.remove-button").data('userid');
            ...
            if (selectedUser == undefined || selectedUser == "") {
                ...
                $.ajax({
                    ...
                    success:    function( data ) {
                                    ...
                                    var notify_type     = '';
                                    if (!data.status) {
                                        notify_icon     = 'fa fa-times';
                                        notify_message  = data.msg + "<br/>" + data.error;
                                        notify_type     = 'danger';
                                    } else {
                                        userTable.row(targetElement).remove().draw();

                                        targetElement   = null;
                                        
                                        notify_icon     = 'fa fa-check';
                                        notify_message  = data.msg;
                                        notify_type     = 'success';
                                    }
